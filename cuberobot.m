classdef cuberobot < handle
    ...
    methods
        ...
        function rotatecube(cube,rotatepoint,rotatedirection,fai)
            %ROTATECUBE Realize the process of cube rotation
            %
            trans = Trans(rotatepoint);
            distrans = Trans(-rotatepoint);
            crotc = CrotC(rotatedirection,fai);
            
            tempapex = [cube.apex;1 1 1 1 1 1 1 1];
            tempapex = trans*crotc*distrans*tempapex;
            cube.apex = tempapex(1:3,:);
            
            tempcenter = [cube.center;1];
            tempcenter = trans*crotc*distrans*tempcenter;
            cube.center = tempcenter(1:3,:);
        end
        
        function rotatecubeplot(cube,rotatepoint,rotatedirection,fai)
            temp = fai/20;
            for i = 2:1:20
                hold off;
                rotatecube(cube,rotatepoint,rotatedirection,temp);
                axis([0,50,0,50,0,50]);
                cubeplot(cube);
            end
        end
